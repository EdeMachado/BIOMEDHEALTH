...
| Data | 2026-08-06 |
| Baseline `main` | `ee0eff64d5ccec04314dd52f1b795fe55e8aaa3c` (PR #54 WP-04.3; closeout #53 `a21a184…`; WP-04.2 `cc62520…` / #52) |
...
| Em curso | Sem WP funcional novo; próximo residual estrutural recomendado: **gap clínico `unit_id`**; **D02-A BLOCKED** |
| Engineering Book | `docs/BIOMED_HEALTH_ENGINEERING_BOOK_v1.md` |

Complemento WP-04.1 (mergeado + HML): policies JWT-era residual → `app_auth`; search_path 0017 endurecido; sinks E01 consent/clínico. Complemento WP-04.2 (**DONE** + HML **0022**): sanitizer, sinks coletivos, append-only/`correlation_id`. Complemento WP-04.3 (**DONE**, PR #54, sem migration / sem HML): pré-auth limite honesto; LGPD sem falso sucesso; provenance RLS; care-plan fine-grained; inventário final. **Não altera** os princípios deste baseline. E01 **não** declarado 100%.
...
WP-04.1 **concluído** (HML **0021**). WP-04.2 **DONE** (HML **0022**). WP-04.3 **DONE** (PR #54 `ee0eff6…`; sem HML). Próximo residual estrutural recomendado: gap clínico `unit_id`. **D02-A BLOCKED.**
