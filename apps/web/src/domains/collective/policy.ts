...
const COLLECTIVE_WRITE_ROLES: ReadonlySet<Role> = new Set([
...
export function canWriteCollective(role: Role | null | undefined): boolean {
  return role ? COLLECTIVE_WRITE_ROLES.has(role) : false;
}

export function sanitizeCollectiveMessage(error: CollectiveError): string {
  switch (error.code) {
    case 'NO_SESSION':
...
export function formatCollectiveScopeLabel(scope: {
  scopeType: string;
  unitId?: string | null;
  ...
  if (scope.unitApplicability === 'selected_units') {
    return 'Organizacao (unidades selecionadas)';
  }
  return 'Organizacao (todas as unidades)';
}

export function formatCollectivePeriod(startsAt: string, endsAt: string): string {
